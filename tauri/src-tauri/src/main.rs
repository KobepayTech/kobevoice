...
/// Find a kobevoice-server process listening on a given port (Windows only).
///
/// Uses PowerShell `Get-NetTCPConnection` to look up the PID owning the port,
/// then verifies via `tasklist` that it's a kobevoice process. The caller is
...
#[cfg(windows)]
fn find_kobevoice_pid_on_port(port: u16) -> Option<u32> {
...
/// Check if a Kobevoice server is responding on the given port.
///
/// Sends an HTTP GET to `/health` and returns `true` only if the response
/// is valid JSON matching the Kobevoice `HealthResponse` schema — specifically
        ...
        .build()
    {
        Ok(client) => match client.get(&url).send() {
            Ok(resp) => {
                ...
                // Parse as JSON and validate Kobevoice-specific fields
...
async fn start_server(
    ...
    // Check if a kobevoice server is already running on our port (from previous session with keep_running=true,
            ...
            .output()
        {
            ...
            for line in output_str.lines().skip(1) {
                ...
                if parts.len() >= 2 {
                    let command = parts[0];
                    let pid_str = parts[1];
                    if command.contains("kobevoice") {
                        if let Ok(pid) = pid_str.parse::<u32>() {
                            println!("Found existing kobevoice-server on port {} (PID: {}), reusing it", SERVER_PORT, pid);
                            // Store the PID so we can kill it on exit if needed
                            *state.server_pid.lock().unwrap() = Some(pid);
                            return Ok(format!("http://127.0.0.1:{}", SERVER_PORT));
                        }
                    } else {
                        // Process name doesn't contain "kobevoice" — could be an external
                        // Python/uvicorn/Docker server. Verify via HTTP health check.
                        println!("Port {} in use by '{}' (PID: {}), checking if it's a Kobevoice server...", SERVER_PORT, command, pid_str);
                        if check_health(SERVER_PORT) {
                            println!("Health check passed — reusing external server on port {}", SERVER_PORT);
                            return Ok(format!("http://127.0.0.1:{}", SERVER_PORT));
                        }
                        println!("Health check failed — port is occupied by a non-Kobevoice process");
                        return Err(format!(
                            "Port {} is already in use by another application ({}). \
                             Close it or change the Kobevoice server port.",
                            SERVER_PORT, command
                        ));
                    }
                }
    ...
    #[cfg(windows)]
    {
        ...
        if TcpStream::connect_timeout(
            ...
            // Port is in use — check if it's a kobevoice process by name first
            if let Some(pid) = find_kobevoice_pid_on_port(SERVER_PORT) {
                println!("Found existing kobevoice-server on port {} (PID: {}), reusing it", SERVER_PORT, pid);
                *state.server_pid.lock().unwrap() = Some(pid);
                return Ok(format!("http://127.0.0.1:{}", SERVER_PORT));
            }
            ...
            println!("Port {} in use by unknown process, checking if it's a Kobevoice server...", SERVER_PORT);
            if check_health(SERVER_PORT) {
                println!("Health check passed — reusing external server on port {}", SERVER_PORT);
                return Ok(format!("http://127.0.0.1:{}", SERVER_PORT));
            }
            return Err(format!(
                "Port {} is already in use by another application. \
                 Close the other application or change the Kobevoice port.",
                SERVER_PORT
            ));
        }
    }

    // Kill any orphaned kobevoice-server from previous session on legacy port 8000
                ...
                if parts.len() >= 2 {
                    ...
                    if command.contains("kobevoice") {
                        if let Ok(pid) = pid_str.parse::<i32>() {
                            println!("Found orphaned kobevoice-server on legacy port {} (PID: {}, CMD: {}), killing it...", LEGACY_PORT, pid, command);
                        ...
                        println!("Legacy port {} is in use by non-kobevoice process: {} (PID: {}), not killing", LEGACY_PORT, command, pid_str);
                    }
                }
            }
        }
    ...
    #[cfg(windows)]
    {
        ...
        if TcpStream::connect_timeout(
            ...
            if let Some(pid) = find_kobevoice_pid_on_port(LEGACY_PORT) {
                println!("Found orphaned kobevoice-server on legacy port {} (PID: {}), killing it...", LEGACY_PORT, pid);
    ...
    println!("=================================================================");
    println!("Starting kobevoice-server sidecar");
    ...
    let cuda_binary = {
        ...
        let cuda_name = if cfg!(windows) {
            "kobevoice-server-cuda.exe"
        } else {
            "kobevoice-server-cuda"
        };
        ...
        if exe_path.exists() {
            ...
            {
                Ok(output) => {
                    // Output format: "kobevoice-server X.Y.Z\n"
    ...
    let sidecar_result = app.shell().sidecar("kobevoice-server");
    ...
    let spawn_result = if let Some(ref cuda_path) = cuda_binary {
        ...
        if let Some(ref dir) = effective_models_dir {
            cmd = cmd.env("KOBEVOICE_MODELS_DIR", dir);
        }
        cmd.spawn()
    } else {
        ...
        if let Some(ref dir) = effective_models_dir {
            sidecar = sidecar.env("KOBEVOICE_MODELS_DIR", dir);
        }
        ...
        match tokio::time::timeout(tokio::time::Duration::from_millis(100), rx.recv()).await {
            ...
            Ok(None) => {
                ...
                #[cfg(not(debug_assertions))]
                {
                    ...
                    eprintln!("Check Console.app logs for more details (search for 'kobevoice')");
...
/// Identifier of the Kobevoice app itself — used to short-circuit auto-paste
/// when the user fires a chord while focus was inside one of our own
/// windows. Paste into Kobevoice-internal targets is step 6 territory and
...
#[cfg(target_os = "macos")]
const KOBEVOICE_BUNDLE_ID: &str = "com.kobepaytech.kobevoice";
#[cfg(target_os = "windows")]
const KOBEVOICE_BUNDLE_ID: &str = "kobevoice.exe";
#[cfg(not(any(target_os = "macos", target_os = "windows")))]
const KOBEVOICE_BUNDLE_ID: &str = "kobevoice";
...
///
/// On macOS this is the call that triggers the "Kobevoice would like to receive
...
/// - `focus.bundle_id` is Kobevoice itself — step 6 will inject directly
...
async fn paste_final_text(
    ...
    if focus.bundle_id.as_deref() == Some(KOBEVOICE_BUNDLE_ID) {
        return Ok(false);
    }
    if !accessibility::is_trusted() {
        return Err(
            "Accessibility permission required for auto-paste. Open System Settings → Privacy & Security → Accessibility and enable Kobevoice."
                .into(),
        );
    }
    ...
    if safe_to_restore {
        ...
        eprintln!(
            "[kobevoice] clipboard mutated during paste window — skipping restore to preserve newer content"
        );
    }
...
async fn debug_focus_roundtrip(
    ...
    if !accessibility::is_trusted() {
        return Err(
            "Accessibility permission not granted. Open System Settings → Privacy & Security → Accessibility and enable Kobevoice."
                .into(),
        );
    }
...
async fn debug_paste_text(
    ...
    if !accessibility::is_trusted() {
        return Err(
            "Accessibility permission not granted. Open System Settings → Privacy & Security → Accessibility and enable Kobevoice, then try again."
                .into(),
        );
    }
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            #[cfg(desktop)]
            {
                ...
                // steals focus to the Kobevoice app when the user clicks
                ...
                // Agent-initiated speech (kobevoice.speak over MCP or POST /speak)
