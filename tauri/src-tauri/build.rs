...
fn main() {
    ...
    #[cfg(target_os = "macos")]
    {
        // kobevoice.icon is in tauri/assets/kobevoice.icon (one level up from src-tauri)
        let icon_source = format!("{}/../assets/kobevoice.icon", project_root);
        ...
        if std::path::Path::new(&icon_source).exists() {
            ...
            // Generate kobevoice.icns from the source PNG via sips + iconutil
            let icns_path = format!("{}/kobevoice.icns", gen_dir);
            if !std::path::Path::new(&icns_path).exists() {
                let source_png = format!("{}/Assets/Kobevoice.png", icon_source);
                if std::path::Path::new(&source_png).exists() {
                    let iconset_dir = format!("{}/kobevoice.iconset", gen_dir);
                    ...
                    match iconutil_output {
                        Ok(out) if out.status.success() => {
                            println!("Generated kobevoice.icns");
                        }
                        Ok(out) => {
    ...
    {
        let required = ["Assets.car", "kobevoice.icns", "partial.plist"];
