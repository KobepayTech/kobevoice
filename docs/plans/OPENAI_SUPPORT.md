...
**Issue:** [#10 OpenAI API compatibility](https://github.com/kobepaytech/kobevoice/issues/10)

## Overview

This feature exposes OpenAI-compatible endpoints from Kobevoice, allowing any tool, library, or application that speaks the OpenAI Audio API to use Kobevoice as a drop-in local replacement.
...
flowchart LR
    ...
    subgraph kobevoice [Kobevoice Server]
...
- **OpenAI SDK users**: `openai.audio.speech.create()` works with Kobevoice
- **LLM frameworks**: LangChain, AutoGen, etc. can use Kobevoice for TTS
...
- `voice` parameter maps to Kobevoice profile names (case-insensitive)
...
async def resolve_voice_for_openai(voice: str, db: Session) -> Optional[VoiceProfile]:
    """
    Resolve OpenAI voice parameter to a Kobevoice profile.
