---
title: "TTS Engines"
description: "How to add new text-to-speech engines to Kobevoice"
---
...
- **`kobevoice-server`** (CPU) — excludes all `nvidia.*` packages to avoid bundling ~3 GB of CUDA DLLs
- **`kobevoice-server-cuda`** — includes `torch.cuda` and `torch.backends.cudnn`
...
The [`docs/PROJECT_STATUS.md`](https://github.com/kobepaytech/kobevoice/blob/main/docs/PROJECT_STATUS.md) file is the canonical, living list of candidates under evaluation — including why some have been backlogged (e.g. VoxCPM, which is effectively CUDA-only upstream).
