---
title: "Building"
description: "How Kobevoice is built for production"
---

## Overview

Kobevoice uses a two-stage build process:
...
cp dist/kobevoice-server ../tauri/src-tauri/binaries/kobevoice-server-${PLATFORM}
```
...
```json
{
  "tauri": {
    "bundle": {
      "externalBin": ["binaries/kobevoice-server"]
    }
  }
}
```

Tauri looks for `kobevoice-server-${PLATFORM}` in `src-tauri/binaries/` and bundles it.
...
3. Package with `scripts/package_cuda.py` into two archives:
   - `kobevoice-server-cuda.tar.gz` — server core (~945 MB)
...
<AccordionGroup>
  ...
  <Accordion title="Tauri can't find sidecar">
    Ensure binary exists at `tauri/src-tauri/binaries/kobevoice-server-${PLATFORM}` before running Tauri build.
  </Accordion>
</AccordionGroup>
