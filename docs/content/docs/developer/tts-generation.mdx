...
description: "How text-to-speech generation works across Kobevoice's multi-engine backend"
---

## Overview

Kobevoice ships seven TTS engines — Qwen3-TTS, Qwen CustomVoice, LuxTTS, Chatterbox Multilingual, Chatterbox Turbo, TADA, and Kokoro — behind a single `TTSBackend` Protocol. All of them expose the same async interface so the routes and services don't need per-engine branching.
...
1. `KOBEVOICE_FORCE_CPU` environment override
