---
title: "Auto-Updater"
description: "How Kobevoice automatic updates work"
---

## Overview

Kobevoice uses Tauri's built-in auto-updater to deliver signed updates to users. The system verifies updates cryptographically before installation.

## How It Works

When Kobevoice launches (in production Tauri builds only), it checks GitHub Releases for a `latest.json` manifest. If a newer version is available:
...
```json
{
  "plugins": {
    "updater": {
      ...
      "dialog": false,
      "endpoints": [
        "https://github.com/kobepaytech/kobevoice/releases/latest/download/latest.json"
      ],
...
```json
{
  ...
  "platforms": {
    "darwin-aarch64": {
      ...
      "url": "https://github.com/kobepaytech/kobevoice/releases/download/v0.2.0/kobevoice_0.2.0_aarch64.app.tar.gz"
    },
    "darwin-x86_64": {
      "signature": "base64_encoded_signature",
      "url": "https://github.com/kobepaytech/kobevoice/releases/download/v0.2.0/kobevoice_0.2.0_x64.app.tar.gz"
    },
    "linux-x86_64": {
      "signature": "base64_encoded_signature",
      "url": "https://github.com/kobepaytech/kobevoice/releases/download/v0.2.0/kobevoice_0.2.0_amd64.AppImage"
    },
    "windows-x86_64": {
      "signature": "base64_encoded_signature",
      "url": "https://github.com/kobepaytech/kobevoice/releases/download/v0.2.0/kobevoice_0.2.0_x64_en-US.msi"
    }
  }
}
```
...
1. **Generate keys** (one-time setup):
   ```bash
   bun tauri signer generate -w ~/.tauri/kobevoice.key
   ```
   This creates:
   - Private key: `~/.tauri/kobevoice.key` (stored in GitHub Secrets, never committed)
   - Public key: `~/.tauri/kobevoice.key.pub` (pasted into `tauri.conf.json`)
...
- `TAURI_SIGNING_PRIVATE_KEY` - Content of `~/.tauri/kobevoice.key`
...
- CUDA server core: ~945 MB (versioned with each Kobevoice release)
...
- **Server core** (`kobevoice-server-cuda.tar.gz`) — the Python server + PyTorch code, changes every release.
...
This means most Kobevoice upgrades only re-download the ~945 MB server core, not the full ~2.5 GB bundle.
...
5. **Restart** — The Kobevoice server restarts and swaps in the CUDA backend.
...
{data_dir}/backends/cuda/
  kobevoice-server-cuda.exe    # Windows
  kobevoice-server-cuda        # macOS/Linux
...
event: progress
data: {"current": 52428800, "total": 945000000, "filename": "kobevoice-server-cuda.tar.gz", "status": "downloading"}
```
...
- `kobevoice-server-cuda.tar.gz` — server-core archive
- `kobevoice-server-cuda.tar.gz.sha256` — checksum
