...
description: "Common issues and solutions for Kobevoice"
---

This guide covers common issues you might encounter when using or developing Kobevoice, along with solutions.
...
xattr -cr /Applications/Kobevoice.app
```
...
<AccordionGroup>
  ...
  <Accordion title="Permission Issues">
    ...
    ```bash
    # macOS/Linux
    chmod +x ~/Library/Application\ Support/com.kobepaytech.kobevoice/backend/kobevoice-server
    ```
  </Accordion>
  ...
  <Accordion title="Check Logs">
    ...
    **macOS:**
    ```bash
    tail -f ~/Library/Application\ Support/com.kobepaytech.kobevoice/logs/server.log
    ```

    **Windows:**
    ```bash
    type %APPDATA%\com.kobepaytech.kobevoice\logs\server.log
    ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Install flash-attn on Windows (community wheels)">
    ...
    Alternatively, run Kobevoice's backend inside WSL2 and use the standard Linux wheels.
  </Accordion>
</AccordionGroup>
...
**Solutions:**
- Close all Kobevoice instances
- Delete the lock file:
  ```bash
  # macOS
  rm ~/Library/Application\ Support/com.kobepaytech.kobevoice/data/kobevoice.db-shm
  rm ~/Library/Application\ Support/com.kobepaytech.kobevoice/data/kobevoice.db-wal
  ```
...
```bash
# macOS
rm ~/Library/Application\ Support/com.kobepaytech.kobevoice/data/kobevoice.db

# Windows
del %APPDATA%\com.kobepaytech.kobevoice\data\kobevoice.db
```
...
<AccordionGroup>
  ...
  <Accordion title="Check GPU Selection">
    If you have multiple GPUs, ensure Kobevoice is using the right one.
...
1. **Check GitHub Issues:** [github.com/kobepaytech/kobevoice/issues](https://github.com/kobepaytech/kobevoice/issues)
2. **Open a New Issue:** Provide:
   - Operating system and version
   - Kobevoice version
   - Steps to reproduce
   - Error messages or logs
3. **Join Discord:** [discord.gg/kobevoice](https://discord.gg/kobevoice) (coming soon)
