...
The model runs through the same backend split Kobevoice already uses for TTS
...
<Steps>
  ...
  <Step title="Speak in character — toggle persona rewrite">
    ...
    generate — Kobevoice runs the text through the personality LLM first,
...
- **Agents that speak in a voice you own.** Combine the persona toggle with
  ...
  personality. The agent calls `kobevoice.speak({ text, profile, personality:
  true })` and Kobevoice rewrites the text in character before speaking.
...
`kobevoice.speak` tool uses when you pass `personality: true`. Scripts and
