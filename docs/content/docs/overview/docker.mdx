...
description: "Run Kobevoice as a headless server with a web UI using Docker"
---

## Overview

Kobevoice can run as a Docker container with a full web UI -- no desktop app required. This is ideal for headless servers, shared GPU machines, or self-hosted deployments.
...
Open [http://localhost:17493](http://localhost:17493) in your browser. The full Kobevoice UI is served directly from the backend.
...
services:
  kobevoice:
    ...
    volumes:
      ...
      - huggingface-cache:/home/kobevoice/.cache/huggingface
    environment:
      - LOG_LEVEL=info
    deploy:
      resources:
...
| `KOBEVOICE_MODELS_DIR` | (HuggingFace cache) | Custom path for model storage |
| `KOBEVOICE_CORS_ORIGINS` | (local origins) | Additional CORS origins, comma-separated |
...
| `kobevoice-data` | `/app/data` | Profiles, database, cache |
| `huggingface-cache` | `/home/kobevoice/.cache/huggingface` | Downloaded models (persists across rebuilds) |
...
- **Non-root user** -- the server runs as `kobevoice`, not `root`
...
For production deployments, put Kobevoice behind nginx or Caddy with TLS and authentication:

```nginx
server {
    listen 443 ssl;
    server_name kobevoice.example.com;

    ssl_certificate /etc/ssl/certs/kobevoice.pem;
    ssl_certificate_key /etc/ssl/private/kobevoice.key;

    auth_basic "Kobevoice";
...
If you see `{"message": "kobevoice API", ...}` instead of the web UI, the frontend build may have failed during the Docker build. Check the build logs:
...
docker run -p 17493:17493 ghcr.io/kobepaytech/kobevoice:latest
```
