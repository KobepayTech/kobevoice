...
a chord, talk, release — Kobevoice transcribes what you said with Whisper,
...
<Callout type="info">
  ...
  per-profile personality modes. It's the "input" half of Kobevoice's voice I/O
...
<Steps>
  ...
  <Step title="Text lands in your app">
    If auto-paste is enabled and Kobevoice has Accessibility permission, the
...
Kobevoice ships two chord behaviors out of the box:
...
- **Left vs right modifier badges.** When you hold keys into the chord
  picker, Kobevoice records whether each modifier is the left or right variant.
...
Once transcription finishes, Kobevoice can synthesize a native paste into
...
- Whisper loop hallucinations — Kobevoice strips repeated tokens (six or
...
- **UAC / UIPI caveat.** Synthetic paste into an *elevated* window from a
  non-elevated Kobevoice is blocked by Windows itself. Run Kobevoice elevated
...
A few cases where Kobevoice deliberately does *not* synthesize a paste:

- **Focus was inside Kobevoice** when the chord started. The transcript goes
  to the Captures tab so a dictation-into-Kobevoice round-trip doesn't
