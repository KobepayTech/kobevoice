...
if [ ! -f ~/.tauri/kobevoice.key ]; then
  echo "❌ Private key not found at ~/.tauri/kobevoice.key"
  echo "Run: cd tauri && bun tauri signer generate -w ~/.tauri/kobevoice.key"
  exit 1
fi

if [ ! -f ~/.tauri/kobevoice.key.pub ]; then
  echo "❌ Public key not found at ~/.tauri/kobevoice.key.pub"
  exit 1
fi
...
export TAURI_SIGNING_PRIVATE_KEY="$(cat ~/.tauri/kobevoice.key)"
