...
EXPORTS_DIR="tauri/assets/kobevoice_exports"
ICON_BUNDLE="tauri/assets/kobevoice.icon"
ASSETS_DIR="$ICON_BUNDLE/Assets"
ICONS_DIR="tauri/src-tauri/icons"
LANDING_LOGO="landing/public/kobevoice-logo.png"
LANDING_PUBLIC="landing/public"
SOURCE_ICON="$EXPORTS_DIR/kobevoice-iOS-Dark-1024x1024@1x.png"

echo "🎨 Updating all Kobevoice icons from exports..."
echo ""
...
echo "Compiling kobevoice.icon with actool..."
...
if [ -f "tauri/src-tauri/gen/kobevoice.icns" ]; then
  echo "  ✓ kobevoice.icns generated"
else
  echo "  ⚠ Warning: kobevoice.icns not generated (will use fallback)"
fi
...
if [ -f "tauri/src-tauri/gen/kobevoice.icns" ]; then
  cp tauri/src-tauri/gen/kobevoice.icns "$ICONS_DIR/icon.icns"
  echo "  ✓ Copied Liquid Glass compiled icon.icns"
else
  echo "  ⚠ Liquid Glass icon not found, generating fallback icon.icns..."
mkdir -p /tmp/kobevoice-iconset.iconset
sips -s format png -z 16 16 "$SOURCE_ICON" --out /tmp/kobevoice-iconset.iconset/icon_16x16.png 2>/dev/null
sips -s format png -z 32 32 "$SOURCE_ICON" --out /tmp/kobevoice-iconset.iconset/icon_16x16@2x.png 2>/dev/null
sips -s format png -z 32 32 "$SOURCE_ICON" --out /tmp/kobevoice-iconset.iconset/icon_32x32.png 2>/dev/null
sips -s format png -z 64 64 "$SOURCE_ICON" --out /tmp/kobevoice-iconset.iconset/icon_32x32@2x.png 2>/dev/null
sips -s format png -z 128 128 "$SOURCE_ICON" --out /tmp/kobevoice-iconset.iconset/icon_128x128.png 2>/dev/null
sips -s format png -z 256 256 "$SOURCE_ICON" --out /tmp/kobevoice-iconset.iconset/icon_128x128@2x.png 2>/dev/null
sips -s format png -z 256 256 "$SOURCE_ICON" --out /tmp/kobevoice-iconset.iconset/icon_256x256.png 2>/dev/null
sips -s format png -z 512 512 "$SOURCE_ICON" --out /tmp/kobevoice-iconset.iconset/icon_256x256@2x.png 2>/dev/null
sips -s format png -z 512 512 "$SOURCE_ICON" --out /tmp/kobevoice-iconset.iconset/icon_512x512.png 2>/dev/null
  sips -s format png -z 1024 1024 "$SOURCE_ICON" --out /tmp/kobevoice-iconset.iconset/icon_512x512@2x.png 2>/dev/null
  iconutil -c icns /tmp/kobevoice-iconset.iconset -o "$ICONS_DIR/icon.icns"
  rm -rf /tmp/kobevoice-iconset.iconset
