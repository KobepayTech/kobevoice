...
MCP clients identify themselves via an ``X-Kobevoice-Client-Id`` HTTP header
(direct-HTTP clients set it in their MCP config; the stdio shim forwards it
from the ``KOBEVOICE_CLIENT_ID`` env var). Middleware copies the value into a
...
# host-filesystem access to loopback callers (see kobevoice.transcribe).
...
# Endpoints that consume X-Kobevoice-Client-Id for its MCP-semantic
...
#
# - /mcp — FastMCP tool calls (kobevoice.speak, kobevoice.transcribe, …)
...
#   false stamps.
# - /speak — REST mirror of kobevoice.speak for non-MCP agents (shell
...
class ClientIdMiddleware(BaseHTTPMiddleware):
    """Copy X-Kobevoice-Client-Id into a ContextVar and stamp last_seen_at
