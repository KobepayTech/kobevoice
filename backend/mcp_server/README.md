...
The server runs inside the same `uvicorn` process as the rest of Kobevoice
...
`kobevoice-mcp` binary ships inside the Kobevoice.app bundle:
...
| Name | Purpose |
|---|---|
| `kobevoice.speak`          | Speak text in a voice profile. Returns a generation id you can poll. |
| `kobevoice.transcribe`     | Whisper transcription of a base64 blob or an absolute local path. |
| `kobevoice.list_captures`  | Recent captures (dictation / recording / file) with transcripts. |
...
2. Per-client binding keyed by `X-Kobevoice-Client-Id`
...
Point it at the URL, hit "List tools," call `kobevoice.list_profiles`
first to confirm wiring, then `kobevoice.speak` for end-to-end.
...
curl -X POST http://127.0.0.1:17493/speak \
  ...
  -H 'X-Kobevoice-Client-Id: claude-code' \
