...
def find_binary() -> Optional[Path]:
    ...
    candidates = [
        DIST_DIR / "kobevoice-server-cuda" / f"kobevoice-server-cuda{exe}",
        DIST_DIR / f"kobevoice-server{exe}",
    ]
    for c in candidates:
...
def write_reports(
    ...
    lines = [
        f"# Kobevoice E2E — {plat} — {started_at.strftime('%Y-%m-%d %H:%M UTC')}",
        "",
        f"Binary: `{binary}`  ",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Kobevoice E2E model generation test")
    p.add_argument("--binary", type=Path, help="Path to kobevoice-server binary (overrides auto-detect)")
...
def main() -> int:
    ...
    # Tempdir + log path
    data_dir = Path(tempfile.mkdtemp(prefix="kobevoice-e2e-"))
