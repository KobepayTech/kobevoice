<p align="center">
  <img src=".github/assets/icon-dark.webp" alt="Kobevoice" width="120" height="120" />
</p>

<h1 align="center">Kobevoice</h1>
...
<p align="center">
  <a href="https://github.com/kobepaytech/kobevoice/releases">
    <img src="https://img.shields.io/github/downloads/kobepaytech/kobevoice/total?style=flat&color=blue" alt="Downloads" />
  </a>
  <a href="https://github.com/kobepaytech/kobevoice/releases/latest">
    <img src="https://img.shields.io/github/v/release/kobepaytech/kobevoice?style=flat" alt="Release" />
  </a>
  <a href="https://github.com/kobepaytech/kobevoice/stargazers">
    <img src="https://img.shields.io/github/stars/kobepaytech/kobevoice?style=flat" alt="Stars" />
  </a>
  <a href="https://github.com/kobepaytech/kobevoice/blob/main/LICENSE">
    <img src="https://img.shields.io/github/license/kobepaytech/kobevoice?style=flat" alt="License" />
  </a>
  <a href="https://deepwiki.com/kobepaytech/kobevoice">
    <img src="https://img.shields.io/static/v1?label=Ask&message=DeepWiki&color=5B6EF7" alt="Ask DeepWiki" />
  </a>
</p>

<p align="center">
    <a href="https://trendshift.io/repositories/21213" target="_blank"><img src="https://trendshift.io/api/badge/repositories/21213" alt="jamiepine%2Fkobevoice | Trendshift" style="width: 250px; height: 55px;" width="250" height="55"/></a>
</p>

<p align="center">
  <a href="https://kobevoice.app">kobevoice.app</a> •
  <a href="https://docs.kobevoice.app">Docs</a> •
...
<p align="center">
  <a href="https://kobevoice.app">
    <img src="landing/public/assets/app-screenshot-1.webp" alt="Kobevoice App Screenshot" width="800" />
  </a>
</p>

<p align="center">
  <em>Click the image above to watch the demo video on <a href="https://kobevoice.app">kobevoice.app</a></em>
</p>

<br/>

<p align="center">
  <img src="landing/public/assets/app-screenshot-2.webp" alt="Kobevoice Screenshot 2" width="800" />
</p>

<p align="center">
  <img src="landing/public/assets/app-screenshot-3.webp" alt="Kobevoice Screenshot 3" width="800" />
</p>
...
Kobevoice is a **local-first AI voice studio** — a free and open-source alternative to **ElevenLabs** and **WisprFlow** in one app. Clone voices from a few seconds of audio, generate speech in 23 languages across 7 TTS engines, dictate into any text field with a global hotkey, and give any MCP-aware AI agent a voice of your choosing.

The two cloud incumbents sit on opposite halves of the voice I/O loop — ElevenLabs on output, WisprFlow on input. Kobevoice does both, bridges them with a bundled local LLM for refinement and per-profile personas, and runs the whole thing on your machine.
...
- **Agent voice output** — one tool call (`kobevoice.speak`) and any MCP-aware agent (Claude Code, Cursor, Cline) speaks to you in a voice you've cloned
...
| Platform              | Download                                               |
| --------------------- | ------------------------------------------------------ |
| macOS (Apple Silicon) | [Download DMG](https://kobevoice.app/download/mac-arm)   |
| macOS (Intel)         | [Download DMG](https://kobevoice.app/download/mac-intel) |
| Windows               | [Download MSI](https://kobevoice.app/download/windows)   |
| Docker                | `docker compose up`                                    |

> **[View all binaries →](https://github.com/kobepaytech/kobevoice/releases/latest)**

> **Linux** — Pre-built binaries are not yet available. See [kobevoice.app/linux-install](https://kobevoice.app/linux-install) for build-from-source instructions.
...
The other half of the voice I/O loop. Hold a hotkey anywhere on your system, speak, release — on macOS the transcript pastes straight into the focused text field. Or hit the mic on any Kobevoice text input and dictate directly into the app.
...
- **In-app mic button** on every Kobevoice text field — generation form, profile descriptions, story titles, anywhere you'd type
...
Kobevoice runs OpenAI Whisper for transcription — the same model that backs dictation, the Captures tab, and the `/transcribe` API. Running on MLX (Apple Silicon) or PyTorch (CUDA / ROCm / DirectML / CPU) depending on your platform.
...
- **Local capture storage** — original audio and transcript stay in your Kobevoice data directory, with a folder shortcut in Settings
...
- **HTTP + stdio transports** — install as a URL in Claude Code / Cursor / Windsurf / VS Code MCP, or point stdio-only clients at the bundled `kobevoice-mcp` binary
...
Agents can reach the same rewrite path over MCP by passing `personality: true` to `kobevoice.speak`, turning the tool into a text-in → personality-LLM → TTS pipeline. The same LLM backs dictation's refinement step — one LLM in the app, one model cache, one GPU-memory footprint.
...
- Custom models directory via `KOBEVOICE_MODELS_DIR`
...
Kobevoice exposes a REST API for integrating voice I/O into your own apps and agents.
...
curl -X POST http://127.0.0.1:17493/speak \
  -H "Content-Type: application/json" \
  -H "X-Kobevoice-Client-Id: my-script" \
...
Kobevoice ships a built-in **Model Context Protocol** server so any MCP-aware agent (Claude Code, Cursor, Windsurf, Cline, VS Code MCP extensions) can speak, transcribe, and browse captures and profiles.

**Claude Code one-liner:**

```
claude mcp add kobevoice \
  --transport http \
  --url http://127.0.0.1:17493/mcp \
  --header "X-Kobevoice-Client-Id: claude-code"
```
...
```json
{
  "mcpServers": {
    "kobevoice": {
      "url": "http://127.0.0.1:17493/mcp",
      "headers": { "X-Kobevoice-Client-Id": "cursor" }
    }
  }
}
```

**Stdio fallback** for clients that don't speak HTTP MCP — point at the bundled `kobevoice-mcp` binary inside the app:
...
```json
{
  "mcpServers": {
    "kobevoice": {
      ...
      "env": { "KOBEVOICE_CLIENT_ID": "claude-desktop" }
    }
  }
}
```

Four tools ship: `kobevoice.speak`, `kobevoice.transcribe`, `kobevoice.list_captures`, `kobevoice.list_profiles`. Per-client voice bindings are managed in **Kobevoice → Settings → MCP**. See the [full MCP guide](docs/content/docs/overview/mcp-server.mdx) for tool signatures, resolution precedence, the speaking-pill contract, and security notes.
...
| **Mobile companion**               | Control Kobevoice from your phone                                         |
...
```bash
git clone https://github.com/kobepaytech/kobevoice.git
cd kobevoice
...
The repo ships a pre-wired `.mcp.json` at the root — running Claude Code inside this checkout picks up the Kobevoice MCP tools automatically once the dev app is running.
